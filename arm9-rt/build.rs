...
INCLUDE device.x"#
    ...
    println!("cargo:rustc-check-cfg=cfg(armv4t)");
    println!("cargo:rustc-check-cfg=cfg(armv5te)");
    println!("cargo:rustc-check-cfg=cfg(arm9)");
    println!("cargo:rustc-check-cfg=cfg(has_fpu)");

    // ARM9 targets (ARMv4T/ARMv5TE)
    let _target_info = if target.starts_with("armv4t-") {
        println!("cargo:rustc-cfg=arm9");
        println!("cargo:rustc-cfg=armv4t");
        "armv4t"
        ...
        // Non ARM9 target, assume testing
        "unknown"
    };

    // ARM9 doesn't use NVIC, so no interrupt handler table size check needed
    let max_int_handlers = 0;

    if target.ends_with("-eabihf") {
        println!("cargo:rustc-cfg=has_fpu");
    }

    // ARM9 vector table is fixed at 8 entries (32 bytes for vectors + 32 bytes for addresses)
    if max_int_handlers > 0 {
        writeln!(
            f,
            r#"
